...
const hentSkjema = async (orgnummer: string, skjemaId: string): Promise<any> => {
    const url = skjemaPath.replace(":orgnummer", orgnummer).replace(":skjemaId", skjemaId)
    const response = await fetchGet(url);
    await handleResponse(response);
    return await response.json();
};
const lagreSkjema = async (orgnummer: string, skjemaId: string,): Promise<any> => {
    const url = skjemaPath.replace(":orgnummer", orgnummer).replace(":skjemaId", skjemaId)
    const response = await fetchPost(url);
    await handleResponse(response);
    return await response.json();
};
const slettSkjema = async (orgnummer: string, skjemaId: string): Promise<any> => {
    const url = skjemaPath.replace(":orgnummer", orgnummer).replace(":skjemaId", skjemaId)
