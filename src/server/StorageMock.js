...
const listObjects = (userId) => {
  const output = [];
  mockStorage.filter(obj => obj.userId === userId).forEach(obj => {
    output.push({
      userId: obj.userId,
...
const getObject = (userId, objectId) => {
  const found = mockStorage.filter(obj => obj.userId === userId && obj.objectId === objectId);
  return found[0];
};
...
const deleteObject = (userId, objectId) => {
  mockStorage = mockStorage.filter(obj => obj.userId !== userId && obj.objectId !== objectId);
};
